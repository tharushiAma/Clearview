...
from sklearn.metrics import (
    ...
    hamming_loss, roc_auc_score, brier_score_loss, balanced_accuracy_score
)
...
class EvalDataset(Dataset):
    ...
    def compute_conflict_label(label_ids: List[int]) -> int:
        # Conflict only exists between sentiment classes (0, 1, 2).
        # Class 3 (NULL) is a non-mention, not a conflicting sentiment.
        valid = [x for x in label_ids if x != -100 and x != 3]
        if len(valid) < 2:
            return 0
        # If we have multiple distinct sentiments (e.g., Positive and Negative)
    ...
    def __getitem__(self, idx):
        ...
        for l in self.labels[idx]:
            # Similar logic as training: map None/NaN -> 3
            if l in LABEL_MAP:
                label_ids.append(LABEL_MAP[l])
            else:
                s_l = str(l).lower()
                if s_l in ["nan", "none", "null", "", "none"]:
                    label_ids.append(3)
                else:
...
def evaluate_single_pass(model, loader, device, msr_on: bool):
    ...
    preds_before = np.concatenate(all_preds_before, axis=0) if all_preds_before else np.empty((0, len(loader.dataset.aspects)))
...
def compute_absa_metrics(preds: np.ndarray, labels: np.ndarray, aspects: List[str]) -> Dict:
    per_aspect = {}
    macro_f1s_4class = []
    macro_f1s_sentiment = [] # Class 0,1,2 only
    weighted_f1s = []
    balanced_accs = []

    # 1. Systemic: Hamming Loss and EM Ratio
    labels_clean = labels.copy()
    labels_clean[labels_clean == -100] = 3
    # hamming_loss doesn't like [N, A] multiclass. Use manual or flatten.
    h_loss = (labels_clean != preds).mean() 
    em_ratio = np.all(labels_clean == preds, axis=1).mean()
    ...
    for i, asp in enumerate(aspects):
        ...
        # 4-class metrics
        f1_macro_4 = f1_score(y_true, y_pred, average="macro", zero_division=0)
        f1_weighted = f1_score(y_true, y_pred, average="weighted", zero_division=0)
        bal_acc = balanced_accuracy_score(y_true, y_pred)
        
        macro_f1s_4class.append(f1_macro_4)
        ...
        sent_mask = (y_true != 3)  
        if sent_mask.sum() > 0:
            y_t_sent = y_true[sent_mask]
            y_p_sent = y_pred[sent_mask]
            f1_macro_sent = f1_score(y_t_sent, y_p_sent, labels=[0,1,2], average="macro", zero_division=0)
            macro_f1s_sentiment.append(f1_macro_sent)

        # Presence detection
        y_true_bin = (y_true != 3).astype(int)
        y_pred_bin = (y_pred != 3).astype(int)
        f1_presence = f1_score(y_true_bin, y_pred_bin, pos_label=1, zero_division=0)

        # Per-class metrics
        p, r, f1_each, _ = precision_recall_fscore_support(y_true, y_pred, labels=[0,1,2,3], zero_division=0)
        
        # G-Mean calculation (Multiclass)
        # G-Mean = (Product of Recalls)^(1/K)
        recalls = r + 1e-6 # avoid 0
        g_mean = np.exp(np.log(recalls).mean())

        # 4x4 Confusion Matrix
        cm = confusion_matrix(y_true, y_pred, labels=[0,1,2,3])

        per_aspect[asp] = {
            "f1_macro_4class": float(f1_macro_4),
            "f1_weighted_4class": float(f1_weighted),
            "f1_macro_sentiment": float(f1_macro_sent) if sent_mask.sum() > 0 else 0.0,
            "f1_presence": float(f1_presence),
            ...
            "per_class": {
                ...
                "null":     {"precision": float(p[3]), "recall": float(r[3]), "f1": float(f1_each[3])},
            },
            "confusion_matrix": cm.tolist(),
            "labels_order": ["negative", "neutral", "positive", "null"]
        }

    overall_macro_4 = float(np.mean(macro_f1s_4class)) if macro_f1s_4class else 0.0
    overall_macro_sent = float(np.mean(macro_f1s_sentiment)) if macro_f1s_sentiment else 0.0
    overall_weighted = float(np.mean(weighted_f1s)) if weighted_f1s else 0.0
    overall_bal_acc = float(np.mean(balanced_accs)) if balanced_accs else 0.0
    
    return {
        "overall_macro_f1_4class": overall_macro_4, 
        "overall_macro_f1_sentiment": overall_macro_sent,
        "overall_weighted_f1": overall_weighted,
        "overall_balanced_accuracy": overall_bal_acc,
        "hamming_loss": float(h_loss),
        "exact_match_ratio": float(em_ratio),
...
def msr_error_reduction(preds_before, preds_after, labels, aspects):
    ...
    # Calculate Macro-F1 change specifically on conflicted samples
    # We define conflicted if TRUE label has multiple distinct aspects
    conf_mask = []
    for i in range(len(labels)):
        valid = [x for x in labels[i] if x != -100 and x != 3]
        conf_mask.append(len(set(valid)) >= 2)
    conf_mask = np.array(conf_mask)
    
    delta_f1_conf = 0.0
    if conf_mask.sum() > 0:
        y_true_conf = labels[conf_mask]
        y_pb_conf = preds_before[conf_mask]
        y_pa_conf = preds_after[conf_mask]
        
        f1_b = f1_score(y_true_conf.flatten(), y_pb_conf.flatten(), average="macro", zero_division=0)
        f1_a = f1_score(y_true_conf.flatten(), y_pa_conf.flatten(), average="macro", zero_division=0)
        delta_f1_conf = f1_a - f1_b

    # Statistical Test: Wilcoxon on accuracy deltas
    # 1 if correct, 0 if incorrect
    acc_before = (preds_before == labels).all(axis=1).astype(int)
    acc_after = (preds_after == labels).all(axis=1).astype(int)
    p_val = 1.0
    if not np.array_equal(acc_before, acc_after):
        try:
            _, p_val = wilcoxon(acc_before, acc_after)
        except:
            p_val = 1.0
    ...
    return report, total, float(delta_f1_conf), float(p_val)
...
def run_ig_faithfulness(model, tokenizer, texts: List[str], device, max_len: int, topk: int = 10):
    ...
    def forward_func(input_ids, attention_mask, aspect_idx: int, enable_msr: bool):
        ...
        logits = out["aspect_logits"][aspect_idx]  # [B,4]
    ...
    for text in texts:
        ...
        probs_after = probs_after.squeeze(0)  # [A,4]
...
def main(args):
    ...
    # Conflict metrics (Supervisor requested ROC-AUC, Brier)
    ...
    # NEW: Advanced Conflict Metrics
    try:
        roc_auc = roc_auc_score(conf_true, conf_scores)
    except:
        roc_auc = 0.5
    brier = brier_score_loss(conf_true, conf_scores)
    ece = compute_ece(conf_scores, conf_true)
    ...
    # MSR reduction
    msr_report, total_red, delta_f1_conf, wilcoxon_p = msr_error_reduction(preds_before, preds_after, labels_np, ASPECTS)
    ...
    report = {
        ...
        "msr_error_reduction": {
            ...
            "delta_f1_conflicted": delta_f1_conf,
    ...
    print(f"ABSA 4-class overall macro F1  = {absa['overall_macro_f1_4class']:.4f}")
    print(f"ABSA Sentiment overall macro F1 = {absa['overall_macro_f1_sentiment']:.4f}")
