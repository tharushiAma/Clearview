...
class ConflictDetector(nn.Module):
    ...
    """
    def __init__(self, num_aspects: int):
        super().__init__()
        self.num_aspects = num_aspects
        in_dim = num_aspects * 3 + num_aspects + 1
    ...
    def forward(self, aspect_logits: List[torch.Tensor]) -> torch.Tensor:
        probs = [F.softmax(lg, dim=-1) for lg in aspect_logits]  # [B,3]
        probs_cat = torch.cat(probs, dim=-1)                    # [B,A*3]
...
class ImprovedRoBERTaHierarchical(nn.Module):
    def __init__(
        ...
        self.conflict_detector = ConflictDetector(num_aspects)
    ...
    def forward(self, input_ids, attention_mask, enable_msr: bool = True, return_attention: bool = False) -> Dict:
        ...
        res = {"aspect_logits": refined_logits, "conflict_score": conflict_score}
        if return_attention:
            res["aspect_attention"] = aspect_attn
            res["cross_attention"] = cross_attn
        return res
    ...
    def compute_loss(self, forward_output: Dict, labels: torch.Tensor, conflict_labels=None,
        ...
        for i, asp in enumerate(self.aspect_names):
            loss_i = self.aspect_losses[asp](aspect_logits[i], labels[:, i])
            if loss_i.item() > 0:
                aspect_loss += loss_i
                valid_aspects += 1
    ...
    @torch.no_grad()
    def predict(self, input_ids, attention_mask, enable_msr: bool = True):
        self.eval()
